"""
Put the frozen feed-forward reconstructors side by side on one scene:
each one reconstructs from the same context views, renders both those
and the views held out from them, and the three are drawn against the
ground truth.

This is the qualitative counterpart of scripts/eval.py, which measures
the whole test split rather than looking at one capture.
"""
import hydra
import torch
...
OUTPUT_PATH = Path(__file__).resolve().parent.parent / "outputs" / "compare_models.png"
